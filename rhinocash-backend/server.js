...
// No framework, no external packages — Node's built-in http server plus the
// small Router in router.js. Zero `npm install` required.
'use strict';
...
router.get('/api/integrations/status', (req, res) => {
  ...
  res.json({
    ...
    mpesaDetail: { sandbox: mpesa.statusFor('sandbox'), production: mpesa.statusFor('production'), active: mpesa.getActiveEnvironment() },
...
router.post('/api/mpesa/callback/:environment', (req, res) => {
  const mpesa = require('./src/integrations/mpesa');
  ...
  const result = mpesa.recordCallback({
  ...
  logAction(req, { action: result.duplicate ? 'Received duplicate M-Pesa callback (ignored)' : 'Received M-Pesa callback', module: 'mpesa', recordType: 'MpesaCallback', recordId: stk.CheckoutRequestID });
  ...
  if (!result.duplicate) {
    const processResult = mpesa.processCallback(result.id, result.initiatedBy);
    if (processResult.created) {
      logAction(req, { action: 'M-Pesa callback converted to real payment', module: 'mpesa', recordType: 'Payment', recordId: processResult.paymentId, newValue: { checkoutRequestId: stk.CheckoutRequestID } });
    }
  }
...
router.post('/api/mpesa/c2b/confirmation/:environment', (req, res) => {
  ...
  const result = mpesa.recordC2bTransaction({
    transId: body.TransID, environment: env, amount: body.TransAmount, msisdn: body.MSISDN, billRefNumber: body.BillRefNumber,
  });
  logAction(req, { action: result.duplicate ? 'Received duplicate M-Pesa C2B transaction (ignored)' : 'Received M-Pesa C2B transaction', module: 'mpesa', recordType: 'MpesaC2b', recordId: body.TransID });
  if (!result.duplicate) {
    const processResult = mpesa.processC2bTransaction(result.id, null);
    if (processResult.created) {
      logAction(req, { action: 'M-Pesa C2B transaction converted to real payment', module: 'mpesa', recordType: 'Payment', recordId: processResult.paymentId, newValue: { transId: body.TransID, matchMethod: result.matchMethod } });
    }
  }
  res.json({ ResultCode: '0', ResultDesc: 'Accepted' });
});
...
router.post('/api/mpesa/b2c/result/:environment', (req, res) => {
  const mpesa = require('./src/integrations/mpesa');
  const { logAction } = require('./src/audit');
  const env = req.params.environment;
  ...
  const outcome = mpesa.processB2cResult({
  ...
  logAction(req, { action: outcome.disbursed ? 'M-Pesa B2C disbursement confirmed — loan disbursed' : (outcome.failed ? 'M-Pesa B2C disbursement failed' : 'Received M-Pesa B2C result'), module: 'mpesa', recordType: 'Loan', recordId: outcome.loanId || result.ConversationID });
  res.json({ ResultCode: 0, ResultDesc: 'Accepted' });
});
...
router.post('/api/mpesa/b2c/timeout/:environment', (req, res) => {
  const mpesa = require('./src/integrations/mpesa');
  const { logAction } = require('./src/audit');
  const env = req.params.environment;
  if (!['sandbox', 'production'].includes(env)) { res.status(400).json({ ResultCode: 1, ResultDesc: 'Unknown environment' }); return; }
  ...
  if (result.ConversationID) {
    const outcome = mpesa.processB2cTimeout({ conversationId: result.ConversationID });
    logAction(req, { action: 'M-Pesa B2C request timed out', module: 'mpesa', recordType: 'Loan', recordId: outcome.loanId || result.ConversationID });
  }
  res.json({ ResultCode: 0, ResultDesc: 'Accepted' });
});
...
router.post('/api/uploads', (req, res, next) => {
  ...
  requireAuth(req, res, async (err) => {
    ...
    const { logAction } = require('./src/audit');
    logAction(req, { action: 'Uploaded file', module: 'uploads', recordType: 'File', recordId: storedName });
...
router.get('/uploads/:name', (req, res, next) => {
  ...
  requireAuth(req, res, (err) => {
    ...
    const doc = get('SELECT * FROM client_documents WHERE file_path = ?', [filePath]);
    if (doc) {
      const client = get('SELECT * FROM clients WHERE id = ?', [doc.client_id]);
      try {
        assertRecordInScope(req.user, client ? client.branch_id : null, 'client');
      } catch (scopeErr) {
    ...
    const avatarOwner = get('SELECT id FROM users WHERE avatar_path = ?', [filePath]);
    if (avatarOwner) {
...
router.post('/api/users/me/avatar', (req, res, next) => {
  const { requireAuth } = require('./src/middleware');
  requireAuth(req, res, (err) => {
    if (err) return next(err);
...
router.delete('/api/users/me/avatar', (req, res, next) => {
  const { requireAuth } = require('./src/middleware');
  requireAuth(req, res, (err) => {
    if (err) return next(err);
    const { run } = require('./src/db');
