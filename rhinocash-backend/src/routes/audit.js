...
function register(router) {
  router.get('/api/audit-logs', requireAuth, requireModule('audit'), async (req, res) => {
    ...
    const rows = await all(`SELECT * FROM audit_logs WHERE ${clause} ORDER BY created_at DESC LIMIT ?`, [...params, limit]);
