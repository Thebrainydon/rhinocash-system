...
function canDecideOn(actor, requesterRecord) {
  if (actor.id === requesterRecord.user_id) return false; // never your own
  if (hasPermission(actor, 'manage_users')) return true;
  const requester = get('SELECT reporting_manager_id FROM users WHERE id = ?', [requesterRecord.user_id]);
  return !!requester && requester.reporting_manager_id === actor.id;
}

// Ticket visibility per the spec's exact scoping table.
function ticketVisibleTo(user, ticket) {
  ...
  if (['manager', 'regional_manager', 'operational_manager'].includes(user.role_id)) {
    const scope = branchScopeSQL(user);
...
async function notifyTicketParticipant(userId, subject, body, relatedId, actorId) {
  ...
  run('INSERT INTO communication_log (id, channel, template, recipient, subject, status, related_type, related_id, sent_by) VALUES (?,?,?,?,?,?,?,?,?)',
...
function register(router) {
  ...
  router.get('/api/notifications', requireAuth, (req, res) => {
    const rows = all('SELECT * FROM notifications WHERE user_id = ? OR user_id IS NULL ORDER BY created_at DESC LIMIT 50', [req.user.id]);
  ...
  router.post('/api/notifications/:id/read', requireAuth, (req, res, next) => {
    ...
    const notif = get('SELECT * FROM notifications WHERE id = ?', [req.params.id]);
  ...
  // ---- Support tickets ----
  router.get('/api/support-tickets', requireAuth, requireModule('support'), (req, res) => {
    let rows = all('SELECT * FROM support_tickets ORDER BY created_at DESC').filter(t => ticketVisibleTo(req.user, t));
  ...
  router.get('/api/support-tickets/dashboard', requireAuth, requireModule('support'), (req, res) => {
    const visible = all('SELECT * FROM support_tickets').filter(t => ticketVisibleTo(req.user, t));
  ...
  router.get('/api/support-tickets/export', requireAuth, requireModule('support'), (req, res) => {
    let rows = all('SELECT * FROM support_tickets ORDER BY created_at DESC').filter(t => ticketVisibleTo(req.user, t));
    if (req.query.status) rows = rows.filter(t => t.status === req.query.status);
    if (req.query.priority) rows = rows.filter(t => t.priority === req.query.priority);
    if (req.query.category) rows = rows.filter(t => t.category === req.query.category);
  ...
  router.get('/api/support-tickets/:id', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    const comments = all('SELECT * FROM support_ticket_comments WHERE ticket_id = ? ORDER BY created_at ASC', [t.id]);
    // Real activity history — reuses the existing audit log rather than a
    // second, duplicate event table, exactly as instructed.
    const activity = all(`SELECT * FROM audit_logs WHERE record_type = 'Ticket' AND record_id = ? ORDER BY created_at ASC`, [t.id]);
    res.json({ ticket: t, sla: ticketSlaInfo(t), comments, activity });
  });

  router.post('/api/support-tickets', requireAuth, requireModule('support'), (req, res, next) => {
    const b = req.body;
    ...
    if (b.clientId) {
      const client = get('SELECT * FROM clients WHERE id = ?', [b.clientId]);
    ...
    if (b.loanId) {
      const loan = get('SELECT * FROM loans WHERE id = ?', [b.loanId]);
    ...
    run('INSERT INTO support_tickets (id, subject, message, category, priority, created_by, branch_id, client_id, loan_id) VALUES (?,?,?,?,?,?,?,?,?)',
      [id, b.subject, b.message || '', b.category || 'General', b.priority || 'Medium', req.user.id, req.user.branch_id, clientId, loanId]);
    logAction(req, { action: 'Opened support ticket', module: 'support', recordType: 'Ticket', recordId: id, newValue: b.subject });
    res.status(201).json({ ticket: get('SELECT * FROM support_tickets WHERE id = ?', [id]) });
  });

  router.patch('/api/support-tickets/:id', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    ...
    run('UPDATE support_tickets SET status = COALESCE(?, status), priority = COALESCE(?, priority), resolved_at = ? WHERE id = ?', [req.body.status || null, req.body.priority || null, resolvedAt, req.params.id]);
    logAction(req, { action: req.body.priority ? 'Changed ticket priority' : 'Updated support ticket', module: 'support', recordType: 'Ticket', recordId: req.params.id, newValue: req.body.status || req.body.priority });
    if ((req.body.status === 'Resolved' || req.body.status === 'Closed') && t.created_by !== req.user.id) {
      notify(t.created_by, 'system', 'Your ticket was resolved', `"${t.subject}" has been marked ${req.body.status}.`);
      notifyTicketParticipant(t.created_by, 'Your ticket was resolved', `"${t.subject}" has been marked ${req.body.status}.`, t.id, req.user.id);
    }
    res.json({ ticket: get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]) });
  });
  ...
  router.post('/api/support-tickets/:id/reopen', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    const isManagerial = ['admin', 'manager', 'regional_manager', 'operational_manager', 'ceo', 'director'].includes(req.user.role_id);
    ...
    run(`UPDATE support_tickets SET status = 'In Progress', reopened_at = datetime('now'), resolved_at = NULL WHERE id = ?`, [t.id]);
    logAction(req, { action: 'Reopened support ticket', module: 'support', recordType: 'Ticket', recordId: t.id });
    if (t.created_by !== req.user.id) { notify(t.created_by, 'system', 'Your ticket was reopened', `"${t.subject}" has been reopened.`); notifyTicketParticipant(t.created_by, 'Your ticket was reopened', `"${t.subject}" has been reopened.`, t.id, req.user.id); }
    res.json({ ticket: get('SELECT * FROM support_tickets WHERE id = ?', [t.id]) });
  });
  ...
  router.post('/api/support-tickets/:id/escalate', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    ...
    run(`UPDATE support_tickets SET escalated_at = datetime('now') WHERE id = ?`, [t.id]);
    logAction(req, { action: 'Escalated support ticket', module: 'support', recordType: 'Ticket', recordId: t.id, newValue: { priority: t.priority, overdueHours: Math.round(sla.overdueHours) } });
    // Escalates to the real Admin(s) — the one role that always has full
    // ticket visibility regardless of branch/region scope.
    const admins = all(`SELECT id FROM users WHERE role_id = 'admin' AND status = 'Active'`);
    admins.forEach(a => { notify(a.id, 'system', `Ticket escalated: ${t.priority}`, `"${t.subject}" is overdue by ${Math.round(sla.overdueHours)}h and has been escalated.`); notifyTicketParticipant(a.id, `Ticket escalated: ${t.priority}`, `"${t.subject}" is overdue by ${Math.round(sla.overdueHours)}h and has been escalated.`, t.id, req.user.id); });
    res.json({ ticket: get('SELECT * FROM support_tickets WHERE id = ?', [t.id]), sla });
  });

  // Real client/loan linking, after creation — same scope validation as at creation.
  router.post('/api/support-tickets/:id/link', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    ...
    if (req.body.clientId !== undefined) {
      if (req.body.clientId === null) { clientId = null; }
        ...
        const client = get('SELECT * FROM clients WHERE id = ?', [req.body.clientId]);
        if (!client) return next({ status: 400, message: 'clientId does not refer to a real client' });
        assertRecordInScope(req.user, client.branch_id, 'client');
        clientId = client.id;
      }
    ...
    if (req.body.loanId !== undefined) {
      if (req.body.loanId === null) { loanId = null; }
        ...
        const loan = get('SELECT * FROM loans WHERE id = ?', [req.body.loanId]);
        if (!loan) return next({ status: 400, message: 'loanId does not refer to a real loan' });
        if (clientId && loan.client_id !== clientId) return next({ status: 400, message: 'loanId does not belong to the linked client' });
        assertRecordInScope(req.user, loan.branch_id, 'loan');
        loanId = loan.id;
    ...
    run('UPDATE support_tickets SET client_id = ?, loan_id = ? WHERE id = ?', [clientId, loanId, t.id]);
    logAction(req, { action: 'Linked client/loan to support ticket', module: 'support', recordType: 'Ticket', recordId: t.id, newValue: { clientId, loanId } });
    res.json({ ticket: get('SELECT * FROM support_tickets WHERE id = ?', [t.id]) });
  });

  // Assignment — real managerial action, not self-service.
  router.post('/api/support-tickets/:id/assign', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    const isManagerial = ['admin', 'manager', 'regional_manager', 'operational_manager', 'ceo', 'director'].includes(req.user.role_id);
    ...
    const assignee = get('SELECT * FROM users WHERE id = ?', [req.body.assignedTo]);
    if (!assignee) return next({ status: 400, message: 'assignedTo does not refer to a real user' });
    const wasAssigned = t.assigned_to;
    run("UPDATE support_tickets SET assigned_to = ?, status = CASE WHEN status = 'Open' THEN 'In Progress' ELSE status END WHERE id = ?", [assignee.id, t.id]);
    logAction(req, { action: wasAssigned ? 'Reassigned support ticket' : 'Assigned support ticket', module: 'support', recordType: 'Ticket', recordId: t.id, newValue: { assignedTo: assignee.id } });
    if (assignee.id !== req.user.id) { notify(assignee.id, 'system', 'Support ticket assigned to you', `"${t.subject}" was assigned to you.`); notifyTicketParticipant(assignee.id, 'Support ticket assigned to you', `"${t.subject}" was assigned to you.`, t.id, req.user.id); }
    res.json({ ticket: get('SELECT * FROM support_tickets WHERE id = ?', [t.id]) });
  });
  ...
  // collaboratively.
  router.post('/api/support-tickets/:id/comments', requireAuth, requireModule('support'), (req, res, next) => {
    const t = get('SELECT * FROM support_tickets WHERE id = ?', [req.params.id]);
    if (!t) return next({ status: 404, message: 'Ticket not found' });
    if (!ticketVisibleTo(req.user, t)) return next({ status: 403, message: 'You do not have access to this ticket' });
    if (!req.body.message) return next({ status: 400, message: 'message is required' });
    const id = 'tcm_' + crypto.randomUUID();
    run('INSERT INTO support_ticket_comments (id, ticket_id, author_id, message) VALUES (?,?,?,?)', [id, t.id, req.user.id, req.body.message]);
    logAction(req, { action: 'Commented on support ticket', module: 'support', recordType: 'Ticket', recordId: t.id });
    const notifyTarget = t.created_by === req.user.id ? t.assigned_to : t.created_by;
    if (notifyTarget && notifyTarget !== req.user.id) { notify(notifyTarget, 'system', 'New reply on your ticket', `New reply on "${t.subject}".`); notifyTicketParticipant(notifyTarget, 'New reply on your ticket', `New reply on "${t.subject}".`, t.id, req.user.id); }
    res.status(201).json({ comment: get('SELECT * FROM support_ticket_comments WHERE id = ?', [id]) });
  });

  // Real, per-user saved filter presets — small, genuinely persisted server-side.
  router.get('/api/ticket-filter-presets', requireAuth, requireModule('support'), (req, res) => {
    res.json({ presets: all('SELECT * FROM ticket_filter_presets WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]) });
  });
  router.post('/api/ticket-filter-presets', requireAuth, requireModule('support'), (req, res, next) => {
    ...
    run('INSERT INTO ticket_filter_presets (id, user_id, name, filters_json) VALUES (?,?,?,?)', [id, req.user.id, req.body.name, JSON.stringify(req.body.filters)]);
    res.status(201).json({ preset: get('SELECT * FROM ticket_filter_presets WHERE id = ?', [id]) });
  });
  router.delete('/api/ticket-filter-presets/:id', requireAuth, requireModule('support'), (req, res, next) => {
    const p = get('SELECT * FROM ticket_filter_presets WHERE id = ?', [req.params.id]);
    ...
    run('DELETE FROM ticket_filter_presets WHERE id = ?', [req.params.id]);
    res.json({ ok: true });
  });
  ...
  router.get('/api/communication-log', requireAuth, requirePermission('manage_system_settings'), (req, res) => {
    let rows = all('SELECT * FROM communication_log ORDER BY created_at DESC LIMIT 200');
  ...
  router.get('/api/faq', requireAuth, requireModule('support'), (req, res) => {
    let rows = all('SELECT * FROM faq_articles ORDER BY category, question');
  ...
  router.post('/api/faq', requireAuth, requirePermission('manage_system_settings'), (req, res, next) => {
    const b = req.body;
    if (!b.question || !b.answer) return next({ status: 400, message: 'question and answer are required' });
    const id = 'faq_' + crypto.randomUUID();
    run('INSERT INTO faq_articles (id, question, answer, category, created_by) VALUES (?,?,?,?,?)', [id, b.question, b.answer, b.category || 'General', req.user.id]);
    logAction(req, { action: 'Added FAQ article', module: 'support', recordType: 'FaqArticle', recordId: id, newValue: b.question });
    res.status(201).json({ article: get('SELECT * FROM faq_articles WHERE id = ?', [id]) });
  });
  router.delete('/api/faq/:id', requireAuth, requirePermission('manage_system_settings'), (req, res, next) => {
    const a = get('SELECT * FROM faq_articles WHERE id = ?', [req.params.id]);
    if (!a) return next({ status: 404, message: 'Article not found' });
    run('DELETE FROM faq_articles WHERE id = ?', [req.params.id]);
    logAction(req, { action: 'Removed FAQ article', module: 'support', recordType: 'FaqArticle', recordId: req.params.id });
    res.json({ ok: true });
  });

  // ---- Leave ----
  router.get('/api/leave-requests', requireAuth, (req, res) => {
    const mine = req.query.mine === '1';
    if (mine) return res.json({ leaveRequests: all('SELECT * FROM leave_requests WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]) });
    ...
    const rows = hasPermission(req.user, 'manage_users')
      ? all('SELECT * FROM leave_requests ORDER BY created_at DESC LIMIT 100')
      : all('SELECT * FROM leave_requests WHERE user_id IN (SELECT id FROM users WHERE reporting_manager_id = ?) ORDER BY created_at DESC', [req.user.id]);
    res.json({ leaveRequests: rows });
  });
  router.post('/api/leave-requests', requireAuth, (req, res, next) => {
    const b = req.body;
    ...
    run('INSERT INTO leave_requests (id, user_id, leave_type, start_date, end_date, reason) VALUES (?,?,?,?,?,?)',
      [id, req.user.id, b.leave_type, b.start_date, b.end_date, b.reason || null]);
    logAction(req, { action: 'Applied for leave', module: 'staff', recordType: 'Leave', recordId: id, newValue: { type: b.leave_type, start: b.start_date, end: b.end_date } });
    res.status(201).json({ leaveRequest: get('SELECT * FROM leave_requests WHERE id = ?', [id]) });
  });
  router.post('/api/leave-requests/:id/decide', requireAuth, (req, res, next) => {
    const { decision } = req.body; // 'Approved' | 'Rejected'
    if (!['Approved', 'Rejected'].includes(decision)) return next({ status: 400, message: 'decision must be Approved or Rejected' });
    const request = get('SELECT * FROM leave_requests WHERE id = ?', [req.params.id]);
    ...
    if (!canDecideOn(req.user, request)) return next({ status: 403, message: 'You are not authorized to decide on this request — only the requester\'s reporting manager or an authorized administrator can' });
    run('UPDATE leave_requests SET status = ?, decided_by = ?, decided_at = ? WHERE id = ?', [decision, req.user.id, nowIso(), req.params.id]);
    logAction(req, { action: `Leave request ${decision}`, module: 'staff', recordType: 'Leave', recordId: req.params.id });
    notify(request.user_id, 'system', `Leave request ${decision}`, `Your ${request.leave_type} leave (${request.start_date} to ${request.end_date}) was ${decision.toLowerCase()}.`);
    res.json({ leaveRequest: get('SELECT * FROM leave_requests WHERE id = ?', [req.params.id]) });
  });

  // ---- Salary advance ----
  router.get('/api/salary-advances', requireAuth, (req, res) => {
    const mine = req.query.mine === '1';
    if (mine) return res.json({ salaryAdvances: all('SELECT * FROM salary_advance_requests WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]) });
    const rows = hasPermission(req.user, 'manage_users')
      ? all('SELECT * FROM salary_advance_requests ORDER BY created_at DESC LIMIT 100')
      : all('SELECT * FROM salary_advance_requests WHERE user_id IN (SELECT id FROM users WHERE reporting_manager_id = ?) ORDER BY created_at DESC', [req.user.id]);
    res.json({ salaryAdvances: rows });
  });
  router.post('/api/salary-advances', requireAuth, (req, res, next) => {
    const b = req.body;
    if (!b.amount || b.amount <= 0) return next({ status: 400, message: 'amount must be positive' });
    const id = 'sa_' + crypto.randomUUID();
    run('INSERT INTO salary_advance_requests (id, user_id, amount, reason) VALUES (?,?,?,?)', [id, req.user.id, b.amount, b.reason || null]);
    logAction(req, { action: 'Applied for salary advance', module: 'staff', recordType: 'SalaryAdvance', recordId: id, newValue: b.amount });
    res.status(201).json({ salaryAdvance: get('SELECT * FROM salary_advance_requests WHERE id = ?', [id]) });
  });
  router.post('/api/salary-advances/:id/decide', requireAuth, (req, res, next) => {
    const { decision } = req.body;
    if (!['Approved', 'Rejected'].includes(decision)) return next({ status: 400, message: 'decision must be Approved or Rejected' });
    const request = get('SELECT * FROM salary_advance_requests WHERE id = ?', [req.params.id]);
    if (!request) return next({ status: 404, message: 'Salary advance request not found' });
    if (request.status !== 'Pending') return next({ status: 409, message: `Already ${request.status.toLowerCase()}` });
    if (!canDecideOn(req.user, request)) return next({ status: 403, message: 'You are not authorized to decide on this request — only the requester\'s reporting manager or an authorized administrator can' });
    run('UPDATE salary_advance_requests SET status = ?, decided_by = ?, decided_at = ? WHERE id = ?', [decision, req.user.id, nowIso(), req.params.id]);
    logAction(req, { action: `Salary advance ${decision}`, module: 'staff', recordType: 'SalaryAdvance', recordId: req.params.id });
    notify(request.user_id, 'system', `Salary advance ${decision}`, `Your salary advance request of ${request.amount} was ${decision.toLowerCase()}.`);
    res.json({ salaryAdvance: get('SELECT * FROM salary_advance_requests WHERE id = ?', [req.params.id]) });
  });
  ...
  router.get('/api/governance/resolutions', requireAuth, (req, res, next) => {
    ...
    res.json({ resolutions: all('SELECT * FROM board_resolutions ORDER BY created_at DESC LIMIT 100') });
  });
  router.post('/api/governance/resolutions', requireAuth, (req, res, next) => {
    ...
    run('INSERT INTO board_resolutions (id, title, description, status, proposed_by) VALUES (?,?,?,?,?)',
      [id, req.body.title, req.body.description || null, 'Proposed', req.user.id]);
    logAction(req, { action: 'Proposed board resolution', module: 'governance', recordType: 'BoardResolution', recordId: id, newValue: { title: req.body.title } });
    res.status(201).json({ resolution: get('SELECT * FROM board_resolutions WHERE id = ?', [id]) });
  });
  router.post('/api/governance/resolutions/:id/decide', requireAuth, (req, res, next) => {
    ...
    const resolution = get('SELECT * FROM board_resolutions WHERE id = ?', [req.params.id]);
    ...
    if (!['Approved', 'Rejected'].includes(decision)) return next({ status: 400, message: 'decision must be Approved or Rejected' });
    run('UPDATE board_resolutions SET status = ?, decided_by = ?, decided_at = ? WHERE id = ?', [decision, req.user.id, nowIso(), req.params.id]);
    logAction(req, { action: `Board resolution ${decision}`, module: 'governance', recordType: 'BoardResolution', recordId: req.params.id });
    res.json({ resolution: get('SELECT * FROM board_resolutions WHERE id = ?', [req.params.id]) });
  });

  // ---- Governance: Equity Holdings (real, read-mostly ledger — Admin/Director maintain it) ----
  router.get('/api/governance/equity', requireAuth, (req, res, next) => {
    if (!GOVERNANCE_ROLES.includes(req.user.role_id)) return next({ status: 403, message: 'Your role does not have governance visibility' });
    const rows = all('SELECT * FROM equity_holdings ORDER BY percentage DESC');
    const totalPct = rows.reduce((s, r) => s + r.percentage, 0);
    res.json({ holdings: rows, totalPercentage: totalPct });
  });
  router.post('/api/governance/equity', requireAuth, (req, res, next) => {
    ...
    const existingTotal = all('SELECT * FROM equity_holdings').reduce((s, r) => s + r.percentage, 0);
    ...
    run('INSERT INTO equity_holdings (id, holder_name, holder_type, percentage, capital_contributed, notes, recorded_by) VALUES (?,?,?,?,?,?,?)',
      [id, b.holder_name, b.holder_type, b.percentage, b.capital_contributed || null, b.notes || null, req.user.id]);
    logAction(req, { action: 'Recorded equity holding', module: 'governance', recordType: 'EquityHolding', recordId: id, newValue: { holder_name: b.holder_name, percentage: b.percentage } });
    res.status(201).json({ holding: get('SELECT * FROM equity_holdings WHERE id = ?', [id]) });
  });
}
