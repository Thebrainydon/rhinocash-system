...
function ledgerBalance(accountId, branchIds) {
  const account = get('SELECT * FROM gl_accounts WHERE id = ?', [accountId]);
...
function resolveScopeForRequest(req) {
  const userScope = branchIdsInScope(req.user); // null = company-wide authority
...
function assertPeriodOpen(dateStr) {
  ...
  const period = get('SELECT * FROM accounting_periods WHERE id = ?', [key]);
...
function computePAR(scope) {
  ...
  for (const loan of loans) {
    const rows = all('SELECT * FROM loan_schedule WHERE loan_id = ?', [loan.id]);
...
function register(router) {
  ...
  router.get('/api/expenses', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    ...
    const agg = get(`SELECT COUNT(*) as cnt, COALESCE(SUM(amount),0) as total FROM expenses ${where}`, params);
    const rows = all(`SELECT * FROM expenses ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, (page - 1) * limit]);
  ...
  router.post('/api/expenses', requireAuth, requireModule('accounting'), (req, res, next) => {
    const b = req.body;
    if (!b.category || !b.amount || b.amount <= 0) return next({ status: 400, message: 'category and a positive amount are required' });
    const branchId = b.branch_id && (branchIdsInScope(req.user) === null || isBranchAllowed(req.user, b.branch_id)) ? b.branch_id : req.user.branch_id;
    const id = 'exp_' + crypto.randomUUID();
    run('INSERT INTO expenses (id, category, amount, note, branch_id, status, submitted_by) VALUES (?,?,?,?,?,?,?)',
      [id, b.category, b.amount, b.note || null, branchId || null, 'Pending', req.user.id]);
    logAction(req, { action: 'Submitted expense', module: 'accounting', recordType: 'Expense', recordId: id, newValue: { category: b.category, amount: b.amount } });
    res.status(201).json({ expense: get('SELECT * FROM expenses WHERE id = ?', [id]) });
  });

  router.post('/api/expenses/:id/approve', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const exp = get('SELECT * FROM expenses WHERE id = ?', [req.params.id]);
    if (!exp) return next({ status: 404, message: 'Expense not found' });
    assertRecordInScope(req.user, exp.branch_id, 'expense');
    if (exp.status !== 'Pending') return next({ status: 409, message: `Cannot approve an expense in ${exp.status} status` });
    run('UPDATE expenses SET status = ?, approved_by = ? WHERE id = ?', ['Approved', req.user.id, exp.id]);
    logAction(req, { action: 'Approved expense', module: 'accounting', recordType: 'Expense', recordId: exp.id });
    if (exp.submitted_by) notify(exp.submitted_by, 'system', 'Expense approved', `Your ${exp.category} expense (${exp.amount}) was approved.`);
    res.json({ expense: get('SELECT * FROM expenses WHERE id = ?', [exp.id]) });
  });

  router.post('/api/expenses/:id/reject', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const exp = get('SELECT * FROM expenses WHERE id = ?', [req.params.id]);
    if (!exp) return next({ status: 404, message: 'Expense not found' });
    assertRecordInScope(req.user, exp.branch_id, 'expense');
    if (!['Pending', 'Approved'].includes(exp.status)) return next({ status: 409, message: `Cannot reject an expense in ${exp.status} status` });
    run('UPDATE expenses SET status = ?, rejection_reason = ? WHERE id = ?', ['Rejected', req.body.reason || null, exp.id]);
    logAction(req, { action: 'Rejected expense', module: 'accounting', recordType: 'Expense', recordId: exp.id, reason: req.body.reason });
    if (exp.submitted_by) notify(exp.submitted_by, 'system', 'Expense rejected', `Your ${exp.category} expense (${exp.amount}) was rejected.${req.body.reason ? ' Reason: ' + req.body.reason : ''}`);
    res.json({ expense: get('SELECT * FROM expenses WHERE id = ?', [exp.id]) });
  });
  ...
  router.post('/api/expenses/:id/pay', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const exp = get('SELECT * FROM expenses WHERE id = ?', [req.params.id]);
    if (!exp) return next({ status: 404, message: 'Expense not found' });
    assertRecordInScope(req.user, exp.branch_id, 'expense');
    if (exp.status !== 'Approved') return next({ status: 409, message: 'Only an Approved expense can be paid' });
    assertPeriodOpen();
    const fundingAccount = (req.body.account_id && get('SELECT id FROM gl_accounts WHERE id = ?', [req.body.account_id])) ? req.body.account_id : 'bank';
    transaction(() => {
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES ('operating_expense', ?, 0, ?, 'expense', ?, ?, ?)`,
        [exp.amount, `${exp.category} — ${exp.note || ''}`, exp.id, exp.branch_id, req.user.id]);
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES (?, 0, ?, ?, 'expense', ?, ?, ?)`,
        [fundingAccount, exp.amount, `${exp.category} — ${exp.note || ''}`, exp.id, exp.branch_id, req.user.id]);
      run('UPDATE expenses SET status = ?, paid_by = ? WHERE id = ?', ['Paid', req.user.id, exp.id]);
    });
    logAction(req, { action: 'Paid expense', module: 'accounting', recordType: 'Expense', recordId: exp.id, newValue: { account: fundingAccount } });
    res.json({ expense: get('SELECT * FROM expenses WHERE id = ?', [exp.id]) });
  });

  // ==================== Requisitions — Submit -> Manager approval -> Accountant pays ====================
  router.get('/api/requisitions', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const clauses = ['1=1']; const params = [];
    if (scope !== null) {
      if (scope.length === 0) clauses.push('1=0');
    ...
    if (req.query.status) { clauses.push('status = ?'); params.push(req.query.status); }
    const rows = all(`SELECT * FROM requisitions WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC LIMIT 200`, params);
    res.json({ requisitions: rows });
  });

  router.post('/api/requisitions', requireAuth, requireModule('accounting'), (req, res, next) => {
    const b = req.body;
    if (!b.category || !b.amount || b.amount <= 0) return next({ status: 400, message: 'category and a positive amount are required' });
    const id = 'req_' + crypto.randomUUID();
    run('INSERT INTO requisitions (id, category, amount, description, branch_id, status, submitted_by) VALUES (?,?,?,?,?,?,?)',
      [id, b.category, b.amount, b.description || null, req.user.branch_id || null, 'Pending', req.user.id]);
    logAction(req, { action: 'Submitted requisition', module: 'accounting', recordType: 'Requisition', recordId: id, newValue: { category: b.category, amount: b.amount } });
    res.status(201).json({ requisition: get('SELECT * FROM requisitions WHERE id = ?', [id]) });
  });
  ...
  router.post('/api/requisitions/:id/decide', requireAuth, (req, res, next) => {
    const reqn = get('SELECT * FROM requisitions WHERE id = ?', [req.params.id]);
    ...
    assertRecordInScope(req.user, reqn.branch_id, 'requisition');
    run("UPDATE requisitions SET status = ?, approved_by = ?, approved_at = datetime('now'), decision_reason = ? WHERE id = ?",
      [decision, req.user.id, req.body.reason || null, reqn.id]);
    logAction(req, { action: `Requisition ${decision.toLowerCase()}`, module: 'accounting', recordType: 'Requisition', recordId: reqn.id, reason: req.body.reason });
    if (reqn.submitted_by) notify(reqn.submitted_by, 'system', `Requisition ${decision.toLowerCase()}`, `Your ${reqn.category} requisition (${reqn.amount}) was ${decision.toLowerCase()}.`);
    res.json({ requisition: get('SELECT * FROM requisitions WHERE id = ?', [reqn.id]) });
  });

  // Accountant/Admin turns an Approved requisition into a real, paid
  // expense — one real financial-record mechanism, not two.
  router.post('/api/requisitions/:id/pay', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const reqn = get('SELECT * FROM requisitions WHERE id = ?', [req.params.id]);
    if (!reqn) return next({ status: 404, message: 'Requisition not found' });
    assertRecordInScope(req.user, reqn.branch_id, 'requisition');
    ...
    transaction(() => {
      run('INSERT INTO expenses (id, category, amount, note, branch_id, status, submitted_by, approved_by, paid_by) VALUES (?,?,?,?,?,?,?,?,?)',
      ...
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES ('operating_expense', ?, 0, ?, 'requisition', ?, ?, ?)`,
        [reqn.amount, `${reqn.category} (requisition) — ${reqn.description || ''}`, reqn.id, reqn.branch_id, req.user.id]);
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES (?, 0, ?, ?, 'requisition', ?, ?, ?)`,
        [fundingAccount, reqn.amount, `${reqn.category} (requisition) — ${reqn.description || ''}`, reqn.id, reqn.branch_id, req.user.id]);
      run('UPDATE requisitions SET status = ?, expense_id = ? WHERE id = ?', ['Paid', expId, reqn.id]);
    });
    logAction(req, { action: 'Paid requisition', module: 'accounting', recordType: 'Requisition', recordId: reqn.id, newValue: { expense_id: expId } });
    res.json({ requisition: get('SELECT * FROM requisitions WHERE id = ?', [reqn.id]) });
  });

  router.post('/api/requisitions/:id/cancel', requireAuth, (req, res, next) => {
    const reqn = get('SELECT * FROM requisitions WHERE id = ?', [req.params.id]);
    if (!reqn) return next({ status: 404, message: 'Requisition not found' });
    ...
    run('UPDATE requisitions SET status = ? WHERE id = ?', ['Cancelled', reqn.id]);
    logAction(req, { action: 'Cancelled requisition', module: 'accounting', recordType: 'Requisition', recordId: reqn.id });
    res.json({ ok: true });
  });

  // ==================== Utility Payments — a real expense with utility-specific fields, not a second engine ====================
  router.get('/api/utility-payments', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const clauses = ['1=1']; const params = [];
    if (scope !== null) {
      if (scope.length === 0) clauses.push('1=0');
    ...
    const rows = all(`SELECT * FROM utility_payments WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC LIMIT 200`, params);
    res.json({ utilityPayments: rows });
  });

  router.post('/api/utility-payments', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const b = req.body;
    if (!b.utility_type || !b.amount || b.amount <= 0) return next({ status: 400, message: 'utility_type and a positive amount are required' });
    assertPeriodOpen();
    const branchId = b.branch_id && (branchIdsInScope(req.user) === null || isBranchAllowed(req.user, b.branch_id)) ? b.branch_id : req.user.branch_id;
    const fundingAccount = (b.account_id && get('SELECT id FROM gl_accounts WHERE id = ?', [b.account_id])) ? b.account_id : 'bank';
    const expId = 'exp_' + crypto.randomUUID();
    ...
    transaction(() => {
      ...
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES ('operating_expense', ?, 0, ?, 'utility', ?, ?, ?)`,
        [b.amount, `${b.utility_type} — ${b.provider || ''}`, expId, branchId, req.user.id]);
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES (?, 0, ?, ?, 'utility', ?, ?, ?)`,
        [fundingAccount, b.amount, `${b.utility_type} — ${b.provider || ''}`, expId, branchId, req.user.id]);
      run('INSERT INTO utility_payments (id, utility_type, provider, account_reference, amount, branch_id, payment_method, status, expense_id, paid_by) VALUES (?,?,?,?,?,?,?,?,?,?)',
    ...
    logAction(req, { action: 'Paid utility bill', module: 'accounting', recordType: 'UtilityPayment', recordId: id, newValue: { utility_type: b.utility_type, amount: b.amount } });
    res.status(201).json({ utilityPayment: get('SELECT * FROM utility_payments WHERE id = ?', [id]) });
  });

  // ==================== Accounting Periods ====================
  router.get('/api/accounting/periods', requireAuth, requireModule('accounting'), (req, res) => {
    const rows = all('SELECT * FROM accounting_periods ORDER BY id DESC LIMIT 24');
  ...
  router.post('/api/accounting/periods/:id/close', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    if (!/^\d{4}-\d{2}$/.test(req.params.id)) return next({ status: 400, message: 'period id must look like "2026-09"' });
    const existing = get('SELECT * FROM accounting_periods WHERE id = ?', [req.params.id]);
    if (existing && existing.status === 'Closed') return next({ status: 409, message: 'Period is already closed' });
    if (existing) run("UPDATE accounting_periods SET status='Closed', closed_by=?, closed_at=datetime('now') WHERE id = ?", [req.user.id, req.params.id]);
    else run("INSERT INTO accounting_periods (id, status, closed_by, closed_at) VALUES (?, 'Closed', ?, datetime('now'))", [req.params.id, req.user.id]);
    logAction(req, { action: 'Closed accounting period', module: 'accounting', recordType: 'AccountingPeriod', recordId: req.params.id });
    res.json({ period: get('SELECT * FROM accounting_periods WHERE id = ?', [req.params.id]) });
  });
  ...
  router.post('/api/accounting/periods/:id/reopen', requireAuth, requirePermission('manage_system_settings'), (req, res, next) => {
    const existing = get('SELECT * FROM accounting_periods WHERE id = ?', [req.params.id]);
    ...
    run("UPDATE accounting_periods SET status='Open', reopened_by=?, reopened_at=datetime('now'), reopen_reason=? WHERE id = ?", [req.user.id, req.body.reason, req.params.id]);
    logAction(req, { action: 'Reopened accounting period', module: 'accounting', recordType: 'AccountingPeriod', recordId: req.params.id, reason: req.body.reason });
    res.json({ period: get('SELECT * FROM accounting_periods WHERE id = ?', [req.params.id]) });
  });

  // ==================== Chart of Accounts management ====================
  router.post('/api/accounts', requireAuth, requirePermission('manage_system_settings'), (req, res, next) => {
    const b = req.body;
    ...
    if (get('SELECT id FROM gl_accounts WHERE id = ? OR code = ?', [id, b.code])) return next({ status: 409, message: 'An account with this id/code already exists' });
    run('INSERT INTO gl_accounts (id, code, name, account_type, status) VALUES (?,?,?,?,?)', [id, b.code, b.name, b.account_type, 'Active']);
    logAction(req, { action: 'Created GL account', module: 'accounting', recordType: 'GLAccount', recordId: id, newValue: { code: b.code, name: b.name, account_type: b.account_type } });
    res.status(201).json({ account: get('SELECT * FROM gl_accounts WHERE id = ?', [id]) });
  });

  router.patch('/api/accounts/:id', requireAuth, requirePermission('manage_system_settings'), (req, res, next) => {
    const acct = get('SELECT * FROM gl_accounts WHERE id = ?', [req.params.id]);
    ...
    run(`UPDATE gl_accounts SET ${sets.join(', ')} WHERE id = ?`, params);
    logAction(req, { action: 'Updated GL account', module: 'accounting', recordType: 'GLAccount', recordId: req.params.id, newValue: req.body });
    res.json({ account: get('SELECT * FROM gl_accounts WHERE id = ?', [req.params.id]) });
  });

  router.get('/api/accounts/:id/usage', requireAuth, requireModule('accounting'), (req, res) => {
    const count = get('SELECT COUNT(*) as c FROM journal_entries WHERE account_id = ?', [req.params.id]).c;
    const lastUsed = get('SELECT MAX(entry_date) as d FROM journal_entries WHERE account_id = ?', [req.params.id]).d;
    res.json({ accountId: req.params.id, transactionCount: count, lastUsed, inUse: count > 0 });
  });

  // ==================== Formal Financial Adjustments ====================
  router.get('/api/adjustments', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const clauses = ['1=1']; const params = [];
    if (scope !== null) {
      if (scope.length === 0) clauses.push('1=0');
    ...
    if (req.query.status) { clauses.push('status = ?'); params.push(req.query.status); }
    const rows = all(`SELECT * FROM adjustments WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC LIMIT 200`, params);
    res.json({ adjustments: rows });
  });

  router.post('/api/adjustments', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const b = req.body;
    ...
    if (!get('SELECT id FROM gl_accounts WHERE id = ?', [b.debit_account]) || !get('SELECT id FROM gl_accounts WHERE id = ?', [b.credit_account])) {
    ...
    run('INSERT INTO adjustments (id, reference, reason, debit_account, credit_account, amount, branch_id, note, status, created_by) VALUES (?,?,?,?,?,?,?,?,?,?)',
    ...
    logAction(req, { action: 'Drafted adjustment', module: 'accounting', recordType: 'Adjustment', recordId: id, newValue: { amount: b.amount, reason: b.reason } });
    res.status(201).json({ adjustment: get('SELECT * FROM adjustments WHERE id = ?', [id]) });
  });

  router.post('/api/adjustments/:id/submit', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const adj = get('SELECT * FROM adjustments WHERE id = ?', [req.params.id]);
    ...
    run("UPDATE adjustments SET status = 'Submitted' WHERE id = ?", [adj.id]);
    logAction(req, { action: 'Submitted adjustment', module: 'accounting', recordType: 'Adjustment', recordId: adj.id });
    res.json({ adjustment: get('SELECT * FROM adjustments WHERE id = ?', [adj.id]) });
  });

  // A second, independent approver — never the same person who drafted
  // it — reviews before anything is posted to the real ledger.
  router.post('/api/adjustments/:id/decide', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const adj = get('SELECT * FROM adjustments WHERE id = ?', [req.params.id]);
    if (!adj) return next({ status: 404, message: 'Adjustment not found' });
    ...
    assertRecordInScope(req.user, adj.branch_id, 'adjustment');
    const decision = req.body.decision;
    if (!['Approved', 'Rejected'].includes(decision)) return next({ status: 400, message: 'decision must be Approved or Rejected' });
    run('UPDATE adjustments SET status = ?, approved_by = ? WHERE id = ?', [decision, req.user.id, adj.id]);
    logAction(req, { action: `Adjustment ${decision.toLowerCase()}`, module: 'accounting', recordType: 'Adjustment', recordId: adj.id, reason: req.body.reason });
    res.json({ adjustment: get('SELECT * FROM adjustments WHERE id = ?', [adj.id]) });
  });

  router.post('/api/adjustments/:id/post', requireAuth, requirePermission('post_accounting_entries'), (req, res, next) => {
    const adj = get('SELECT * FROM adjustments WHERE id = ?', [req.params.id]);
    if (!adj) return next({ status: 404, message: 'Adjustment not found' });
    if (adj.status !== 'Approved') return next({ status: 409, message: 'Only an Approved adjustment can be posted' });
    assertPeriodOpen();
    transaction(() => {
      run(`INSERT INTO journal_entries (account_id, debit, credit, description, ref_type, ref_id, branch_id, posted_by) VALUES (?, ?, 0, ?, 'adjustment', ?, ?, ?)`,
      ...
      run("UPDATE adjustments SET status = 'Posted', posted_at = datetime('now') WHERE id = ?", [adj.id]);
    });
    logAction(req, { action: 'Posted adjustment', module: 'accounting', recordType: 'Adjustment', recordId: adj.id, newValue: { debit: adj.debit_account, credit: adj.credit_account, amount: adj.amount } });
    res.json({ adjustment: get('SELECT * FROM adjustments WHERE id = ?', [adj.id]) });
  });
  ...
  router.get('/api/accounting/par', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    res.json(computePAR(scope));
  });

  // ==================== Branch Profitability ====================
  router.get('/api/accounting/branch-profitability', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = branchIdsInScope(req.user);
    let branches = all('SELECT * FROM branches');
    if (scope !== null) branches = branches.filter(b => scope.includes(b.id));
    const result = branches.map(b => {
      const revenue = ledgerBalance('interest_income', [b.id]) + ledgerBalance('fee_income', [b.id]);
      const expenses = ledgerBalance('operating_expense', [b.id]);
      const portfolio = all(`SELECT s.total_due, s.paid_amount FROM loan_schedule s JOIN loans l ON l.id = s.loan_id WHERE l.branch_id = ? AND l.status IN ('Active','Disbursed')`, [b.id])
        .reduce((s, r) => s + Math.max(0, r.total_due - r.paid_amount), 0);
  ...
  router.get('/api/accounting/approval-aging', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const now = Date.now();
    ...
    const pendingExpenses = branchFilter(all("SELECT * FROM expenses WHERE status = 'Pending'"))
    ...
    const pendingRequisitions = branchFilter(all("SELECT * FROM requisitions WHERE status = 'Pending'"))
    ...
    const approvedRequisitions = branchFilter(all("SELECT * FROM requisitions WHERE status = 'Approved'"))
    ...
    const submittedAdjustments = branchFilter(all("SELECT * FROM adjustments WHERE status = 'Submitted'"))
    ...
    const pendingLoans = all(`SELECT id, branch_id, status, created_at FROM loans WHERE ${loanScope} AND status LIKE 'Waiting for%'`, loanParams)
  ...
  // ==================== Chart of Accounts ====================
  router.get('/api/accounts', requireAuth, requireModule('accounting'), (req, res) => {
    let rows = all('SELECT * FROM gl_accounts');
  ...
  router.get('/api/journal-entries', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const clauses = ['1=1']; const params = [];
    if (scope !== null) {
      if (scope.length === 0) clauses.push('1=0');
    ...
    if (req.query.date_from) { clauses.push('date(entry_date) >= date(?)'); params.push(req.query.date_from); }
    if (req.query.date_to) { clauses.push('date(entry_date) <= date(?)'); params.push(req.query.date_to); }
    const page = Math.max(1, parseInt(req.query.page, 10) || 1);
    const limit = Math.min(200, Math.max(1, parseInt(req.query.limit, 10) || 50));
    const where = `WHERE ${clauses.join(' AND ')}`;
    const agg = get(`SELECT COUNT(*) as cnt FROM journal_entries ${where}`, params);
    const rows = all(`SELECT * FROM journal_entries ${where} ORDER BY entry_date DESC, id DESC LIMIT ? OFFSET ?`, [...params, limit, (page - 1) * limit]);
  ...
  router.get('/api/accounting/cash-position', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const accounts = ['cash', 'bank', 'mpesa'];
    const balances = {};
    accounts.forEach(a => { balances[a] = ledgerBalance(a, scope); });
    res.json({ balances });
  });

  router.get('/api/accounting/trial-balance', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const accounts = all('SELECT * FROM gl_accounts');
    const rows = accounts.map(a => ({ account: a.id, name: a.name, type: a.account_type, balance: ledgerBalance(a.id, scope) }));
    ...
    const sums = get(sumSql, params);
    res.json({ rows, totalDebits: sums.d, totalCredits: sums.c, balanced: Math.abs(sums.d - sums.c) < 0.01 });
  });

  router.get('/api/accounting/profit-and-loss', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const interestIncome = ledgerBalance('interest_income', scope);
    const feeIncome = ledgerBalance('fee_income', scope);
    const expenses = ledgerBalance('operating_expense', scope);
    res.json({ interestIncome, feeIncome, expenses, netProfit: interestIncome + feeIncome - expenses });
  });

  router.get('/api/accounting/balance-sheet', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    const assets = ['cash', 'bank', 'mpesa', 'loans_receivable'].map(a => ({ account: a, balance: ledgerBalance(a, scope) }));
    const totalAssets = assets.reduce((s, a) => s + a.balance, 0);
    const liabilities = [{ account: 'overpayment_suspense', balance: ledgerBalance('overpayment_suspense', scope) }];
  ...
  router.get('/api/accounting/cashflow', requireAuth, requireModule('accounting'), (req, res) => {
    const scope = resolveScopeForRequest(req);
    ...
    const openingRow = get(
      `SELECT COALESCE(SUM(debit),0) as d, COALESCE(SUM(credit),0) as c FROM journal_entries WHERE account_id IN (${acctList}) AND date(entry_date) < date(?) ${scopeClause}`,
      [...cashAccounts, from, ...scopeParams]
    );
    const opening = openingRow.d - openingRow.c;

    const periodRow = get(
      `SELECT COALESCE(SUM(debit),0) as d, COALESCE(SUM(credit),0) as c FROM journal_entries WHERE account_id IN (${acctList}) AND date(entry_date) BETWEEN date(?) AND date(?) ${scopeClause}`,
