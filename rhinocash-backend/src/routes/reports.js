...
function resolveReportScope(req) {
  const userScope = branchIdsInScope(req.user); // null = company-wide
...
function loanIdsInScope(scope, extraClause, extraParams) {
  ...
  return all(`SELECT id FROM loans WHERE ${full}`, extraClause ? [...params, ...(extraParams || [])] : params).map(l => l.id);
}
...
function register(router) {
  ...
  router.get('/api/reports/portfolio', requireAuth, requireModule('reports'), (req, res) => {
    const { computePAR } = require('./accounting');
    const scope = resolveReportScope(req);
    const par = computePAR(scope);
    const activeLoanIds = loanIdsInScope(scope, `status IN ('Active','Disbursed')`);
    const disbursedThisMonth = get(
      `SELECT COALESCE(SUM(principal),0) as v, COUNT(*) as c FROM loans WHERE id IN (${activeLoanIds.length ? activeLoanIds.map(() => '?').join(',') : "''"}) AND disbursed_at >= date('now','start of month')`,
      activeLoanIds
    );
    res.json({
  ...
  router.get('/api/reports/collections', requireAuth, requireModule('reports'), (req, res) => {
    const { collectionTotals } = require('./collections');
    const scope = resolveReportScope(req);
    const activeLoanIds = loanIdsInScope(scope, `status IN ('Active','Disbursed')`);
    const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0, 0, 0, 0);
    const today = new Date();
    const { expected, collected } = collectionTotals(activeLoanIds, monthStart.toISOString().slice(0, 10), today.toISOString().slice(0, 10));
    ...
    activeLoanIds.forEach(loanId => {
      const rows = all(`SELECT * FROM loan_schedule WHERE loan_id = ? AND status != 'Paid' AND due_date < date('now')`, [loanId]);
      rows.forEach(r => {
  ...
  router.get('/api/reports/targets', requireAuth, requireModule('reports'), (req, res) => {
    const { computeAchievement } = require('./targets');
    const scope = branchIdsInScope(req.user);
    let targets = all(`SELECT * FROM targets WHERE status = 'Active'`);
    if (scope !== null) {
      targets = targets.filter(t => {
        if (t.branch_id) return scope.includes(t.branch_id);
        if (t.recipient_user_id) { const u = get('SELECT branch_id FROM users WHERE id = ?', [t.recipient_user_id]); return u && scope.includes(u.branch_id); }
        return false;
      });
    }
    const withAchievement = targets.map(t => ({ ...t, ...computeAchievement(t) }));
  ...
  router.get('/api/reports/financial', requireAuth, (req, res, next) => {
    ...
    const revenue = ledgerBalance('interest_income', branchFilter) + ledgerBalance('fee_income', branchFilter);
    const expenses = ledgerBalance('operating_expense', branchFilter);
    res.json({ revenue, expenses, netProfit: revenue - expenses });
  });

  // ==================== Growth (real, from real created_at timestamps) ====================
  router.get('/api/reports/growth', requireAuth, requireModule('reports'), (req, res) => {
    const scope = resolveReportScope(req);
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const newClients = get(`SELECT COUNT(*) as c FROM clients WHERE ${clientClause} AND strftime('%Y-%m', created_at) = ?`, [...clientParams, monthKey]).c;
      const newLoans = get(`SELECT COUNT(*) as c, COALESCE(SUM(principal),0) as v FROM loans WHERE ${clientClause} AND strftime('%Y-%m', created_at) = ?`, [...clientParams, monthKey]);
  ...
  router.get('/api/reports/branch-ranking', requireAuth, requireModule('reports'), (req, res) => {
    const { collectionTotals } = require('./collections');
    const { computePAR } = require('./accounting');
    const { ledgerBalance } = require('./accounting');
    const scope = resolveReportScope(req);
    ...
    const result = branches.map(b => {
      const loanIds = loanIdsInScope([b.id], `status IN ('Active','Disbursed')`);
      const { expected, collected } = collectionTotals(loanIds, monthStart.toISOString().slice(0, 10), today.toISOString().slice(0, 10));
      const par = computePAR([b.id]);
  ...
  router.get('/api/reports/export', requireAuth, requireModule('reports'), (req, res) => {
    const { computePAR } = require('./accounting');
    const { collectionTotals } = require('./collections');
    const scope = resolveReportScope(req);
    const par = computePAR(scope);
  ...
  router.get('/api/report-filter-presets', requireAuth, requireModule('reports'), (req, res) => {
    res.json({ presets: all('SELECT * FROM report_filter_presets WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]) });
  });
  router.post('/api/report-filter-presets', requireAuth, requireModule('reports'), (req, res, next) => {
    ...
    run('INSERT INTO report_filter_presets (id, user_id, name, filters_json) VALUES (?,?,?,?)', [id, req.user.id, req.body.name, JSON.stringify(req.body.filters)]);
    res.status(201).json({ preset: get('SELECT * FROM report_filter_presets WHERE id = ?', [id]) });
  });
  router.delete('/api/report-filter-presets/:id', requireAuth, requireModule('reports'), (req, res, next) => {
    const p = get('SELECT * FROM report_filter_presets WHERE id = ?', [req.params.id]);
    ...
    run('DELETE FROM report_filter_presets WHERE id = ?', [req.params.id]);
    res.json({ ok: true });
  });
}
