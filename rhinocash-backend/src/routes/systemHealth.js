...
async function requireSystemHealthAuth(req, res, next) {
  if (req.user.role_id === 'admin' || req.user.role_id === 'ceo' || (await hasModuleAccess(req.user, 'audit'))) return next();
...
function register(router) {
  ...
  router.get('/api/system/health', requireAuth, requireSystemHealthAuth, async (req, res) => {
    ...
    let dbOk = true;
    try { await get('SELECT 1 as ok'); } catch (e) { dbOk = false; }
    const dbLatencyMs = Date.now() - dbStart;

    const activeUsers = (await get(`SELECT COUNT(*) as c FROM users WHERE status = 'Active'`)).c;
    const inactiveUsers = (await get(`SELECT COUNT(*) as c FROM users WHERE status != 'Active'`)).c;

    const failedLogins24h = (await get(`SELECT COUNT(*) as c FROM login_attempts WHERE success = 0 AND created_at > iso_offset(interval '-24 hours')`)).c;
    const successfulLogins24h = (await get(`SELECT COUNT(*) as c FROM login_attempts WHERE success = 1 AND created_at > iso_offset(interval '-24 hours')`)).c;
    ...
    const notificationsSent24h = (await get(`SELECT COUNT(*) as c FROM notifications WHERE created_at > iso_offset(interval '-24 hours')`)).c;
    const notificationsUnread = (await get(`SELECT COUNT(*) as c FROM notifications WHERE read = 0`)).c;
  ...
  router.get('/api/system/security-events', requireAuth, requireSystemHealthAuth, async (req, res) => {
    ...
    const total = (await get(`SELECT COUNT(*) as c FROM login_attempts WHERE ${clauses.join(' AND ')}`, params)).c;
    const rows = await all(`SELECT * FROM login_attempts WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, (page - 1) * limit]);
