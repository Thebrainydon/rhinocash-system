...
| SQL injection protection | Done | Every query uses parameterized `?` placeholders via `node:sqlite`'s prepared statements — no string-concatenated SQL anywhere in the codebase. |
...
| Database backup strategy | Deployment | SQLite: back up `data/rhinocash.db` on whatever schedule your deployment platform supports (it's a single file). Postgres (once migrated): use your provider's automated backups (Supabase/Neon/RDS all offer this) — do not build a custom backup mechanism into the app. |
| Dependency vulnerabilities | N/A | Zero external dependencies — there is no `node_modules` supply chain to audit. This is a direct consequence of the offline build environment, and also just... one less thing to worry about. |
| Multi-instance rate-limit consistency | Known gap | `src/rateLimit.js` is in-memory per process. Fine for a single instance; documented in `docs/POSTGRES_MIGRATION.md` as needing a shared store once you scale to multiple instances. |
...
| Secrets encrypted at rest (M-Pesa credentials) | Done | AES-256-GCM, `src/crypto.js`. `MPESA_ENCRYPTION_KEY` env var in production, auto-generated+persisted otherwise (same pattern as `SESSION_SECRET`). Verified by directly scanning the raw `.db` file for plaintext secrets in `test/mpesaConfig.test.js` — they're not there. |

## Before going to production, at minimum

1. Set `SESSION_SECRET`, `CORS_ORIGIN`, and `NODE_ENV=production` explicitly.
...
4. Decide whether SQLite-on-a-single-instance is actually sufficient for
   your expected load before treating Postgres migration as optional —
