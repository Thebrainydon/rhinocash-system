...
real running server and a real SQLite database — not a description of
intended behavior.

```
Backend:  776 passed, 0 failed  (24 suites — see test/run-all.sh)
Frontend: 464 passed, 0 failed  (drives the real UI functions in
                                 rhinocash-app/index.html end-to-end
                                 against a live backend — see
                                 test/run-frontend.sh)
Total:    1,240 passed, 0 failed
```
...
built-ins only, zero `npm install`), real frontend (a single-file app
that talks to the backend over `fetch()` for everything, not a
prototype with local/mock state), real SQLite database, covering:
...
adjustment — added specifically so a crash mid-write can never leave a
payment recorded with no matching journal entry, or vice versa).
...
- **Live Safaricom M-Pesa round-trip.** The STK/C2B/B2C code is real and
  ...
  Safaricom's sandbox or production servers has never happened, because
  this was built in an environment with no such network access.
...
- **PostgreSQL.** `src/db.js` is the only file that knows it's SQLite —
  see `docs/POSTGRES_MIGRATION.md` for the scoped migration plan — but
  the swap has not been made or tested against a real Postgres instance.
...
- **`docs/POSTGRES_MIGRATION.md`** — the scoped plan for moving off SQLite.
